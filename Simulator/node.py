...
class Node(Module):
    ...
    def handle_arrival(self):
        ...
        if self.state == Node.IDLE:
            #no signal in the air, then I could tx
            ...
            #signal in the air, I need go to CS
            else:
                self.state = Node.CS
                self.logger.log_state(self, Node.CS)
    ...
    def handle_end_rx(self, event):
        ...
        if self.state == Node.RX:
            ...
            # log packet
            self.logger.log_packet(event.get_source(), self, packet)
            # if i'm in CS and not signal in the air, then I can TX
            if (self.state == Node.CS) and (self.receiving_count == 0):
    ...
    def handle_end_proc(self, event):
        ...
        if len(self.queue) == 0:
            ...
            #if not signal in the air, then can I tx
            if self.receiving_count == 0:
                # there is a packet ready, trasmit it
                packet_size = self.queue.pop(0)
                self.transmit_packet(packet_size)
                ...
                #signal in the air, I need got to CS
                self.state = Node.CS
                self.logger.log_state(self, Node.CS)
